"""TPC-H (PDS) query suite in Trino SQL dialect.

Ported verbatim from the Spark suite in emr_tpch_benchmark.py, with only these
dialect edits: q1 uses `l_shipdate <= date '1998-09-02'` (no Spark date()
wrapper); q18 uses `o_orderdate` and `cast(sum(l_quantity) as double)` in place
of Spark's to_date()/DOUBLE(). No change to query logic.

Disclaimer: derived from the TPC-H benchmark; results are not comparable to
published TPC-H results. See the repo README.
"""
...
QUERIES: dict[int, str] = {
    ...
    18: """
        select
            ...
            cast(sum(l_quantity) as double) as col6
        from customer, orders, lineitem
        where
            o_orderkey in (
